...
class RocketGame:
    ...
    def run_game(self):
        ...
        while True:
            ...
            if self.rocket.remaining > 0:
                self.rocket.update()
                self._update_bullets()
                self._update_aliens()
                self._check_bullet_alien_collisions()
                self._check_destroy_rocket()
    ...
    def _destroy_rocket(self):
        """Destroy the rocket, and if another rocket is available, restart"""
        # Remove rocket from remaining rockets
        self.rocket.remaining -= 1
        ...
        if self.rocket.remaining > 0:
            ...
            # Move rocket to initial position
    ...
    def _check_destroy_rocket(self):
        """Respond to rocket-alien collision and to alien reaching the edge."""
        # Initialize all objects and their position when rocket gets destroyed
        if pygame.sprite.spritecollideany(self.rocket, self.aliens):
            self._destroy_rocket()
        
        # Initialize all objects and their position when alien reaches the edge
